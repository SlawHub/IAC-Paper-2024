...
Node = coe(4,:); ArgP = coe(5,:); MA = coe(6,:); %TA = coe(6,:);  %Indexing Classical Orbital Elements
TA = zeros(length(MA),1);
for k = 1:length(MA)
    TA(k,:) = Kepler(e(k,:),MA(k,:));
end
%% Calculating Parameters
p = a*(1-e ^2); %Semi-latus rectum
r = p/(1 + e*cos(TA)); %Orbit equation (km)
...
R3_Node = [cos(Node) sin(Node) 0 ; -sin(Node) cos(Node) 0 ; 0 0 1]; %Rotation about 3rd axis
R1_i= [1 0 0 ; 0 cos(i) sin(i) ; 0 -sin(i) cos(i)]; %Rotation about 2nd axis
R3_ArgP = [cos(ArgP) sin(ArgP) 0 ; -sin(ArgP) cos(ArgP) 0 ; 0 0 1]; %Rotation about 1st axis
...
% RV = [Perifocal_pos ; Perifocal_vel];
end